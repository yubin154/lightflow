...
public class TestDbFramework extends BaseTestCase {
	...
	public void testTransaction() throws Exception {
		...
		try {
			TransactionManager.execute(dao.getDbEnum(),
				new ITransactional() {
					public void execute() throws Throwable {
						dao.save(data);
						conn[0] = ConnectionHelper.getConnection(dao.getDbEnum());
						// current connection should have autocommit turned off
						Assert.assertFalse(conn[0].getAutoCommit());
						Connection anotherConn = ConnectionHelper.getConnection(dao.getDbEnum());
						Assert.assertTrue((anotherConn == conn[0]));
					}
				}
			);
			// after transaction, autocommit should be turned back on
			Assert.assertTrue(conn[0].getAutoCommit());
		}
		catch (Throwable e) {
			e.printStackTrace();
		}
		long id = data.getId();
		ConnectionHelper.cleanupDBResources(null, null, conn[0]);
		try {
			TransactionManager.execute(dao.getDbEnum(),
				new ITransactional() {
					public void execute() throws Exception {
						dao.delete(data);
						conn[0] = ConnectionHelper.getConnection(dao.getDbEnum());
						// do something wired to break the transaction
						Statement stmt = conn[0].createStatement();
						stmt.executeUpdate("something wrong");
					}
				}
			);
			// after transaction, auto commit should be turned back on
			Assert.assertTrue(conn[0].getAutoCommit());
			// delete transaction rolled back, the data should still be in the table
			Assert.assertEquals(1, AbstractDAO.aggregate(dao.getDbEnum(), new Query().select("COUNT(ID)").from("test_dbframework")));
		}
		catch (Throwable e) {
			// we expecting failure, ignore
		}
		// we have to reload the data since its id has been changed to NULL after delete
		dao.initUnique(data, "ID", new Long(id));
		dao.delete(data);
		Assert.assertEquals(0, AbstractDAO.aggregate(dao.getDbEnum(), new Query().select("COUNT(ID)").from("test_dbframework")));
	}
	...
	public void testLock() throws Exception {
		...
		dao.save(data);
		try {
			dao.search(new Query().and("ID", "=", new Long(data.getId())), 
				new IDataAccess.IResultSetHandler() {
					public void next(Connection conn, ResultSet rs) throws SQLException {
						Assert.assertEquals(true, conn.getAutoCommit());
						Statement stmt = conn.createStatement();
						stmt.executeUpdate("UPDATE test_dbframework SET col_vc='linux' WHERE ID=" + rs.getInt("id"));
						stmt.close();
					}
			...
			SampleDO another = dao.findById(data.getId());
			Assert.assertEquals("linux", another.getColVc());
			dao.delete(another);
		}
		catch (Exception e) {
			e.printStackTrace();
